...
one(){ tag=$1; shift
  env ONNXIM_WEIGHT_SWIZZLE=1 RAMULATOR_REQBUF=1024 RAMULATOR_WEIGHT_LIMIT=500000000 \
      ...
      ./bin/Simulator --config ../configs/_c128.json \
        ...
        > /workspace/ONNXim/out/rowhit/$tag.log 2>&1
  echo "done $tag"; }
one rh_head ONNXIM_KV_LAYOUT=head     ONNXIM_KV_BLOCK=16 &
one rh_k1p64 ONNXIM_KV_LAYOUT=headbank ONNXIM_KV_BANKS_PER_HEAD=1 ONNXIM_KV_BLOCK=64 &
wait
